...
-- 4. Use PARTITION BY constituency to show the ranking of each party in Edinburgh 
-- in 2017. Order your results so the winners are shown first, then ordered by constituency.
...
select  constituency, party from 
  (SELECT constituency, party, votes, rank() OVER (PARTITION BY constituency order by votes desc) ranking
    FROM ge WHERE constituency BETWEEN 'S14000021' AND 'S14000026' AND yr  = 2017 ORDER BY constituency,
     votes DESC) t WHERE ranking = 1;

-- 6. Show how many seats for each party in Scotland in 2017.
SELECT party, COUNT(1) from 
(
SELECT constituency, party, votes, rank() OVER (PARTITION BY constituency ORDER BY votes DESC) ranking
  FROM ge WHERE constituency LIKE 'S%' AND yr  = 2017 ORDER BY constituency,votes DESC
    ) t WHERE ranking = 1 GROUP BY party;
