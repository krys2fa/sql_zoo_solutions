-- 1. The first example shows the goal scored by a player with the 
-- last name 'Bender'. The * says to list all the columns in the 
-- table - a shorter way of saying matchid, teamid, player, gtime
-- Modify it to show the matchid and player name for all goals scored 
-- by Germany. To identify German players, check for: teamid = 'GER'
SELECT matchid, player FROM goal WHERE teamid = 'GER';

-- 2. From the previous query you can see that Lars Bender's scored a 
-- goal in game 1012. Now we want to know what teams were playing in that match.
-- Notice in the that the column matchid in the goal table corresponds to the 
-- id column in the game table. We can look up information about game 1012 by 
-- finding that row in the game table.
-- Show id, stadium, team1, team2 for just game 1012
SELECT id,stadium,team1,team2 FROM game WHERE id = 1012;

-- 3. The code below shows the player (from the goal) and stadium name 
...
SELECT mdate, teamname FROM game
  JOIN eteam ON (team1=eteam.id) WHERE coach = 'Fernando Santos';
...
SELECT stadium, COUNT(1) FROM goal JOIN game ON matchid = id GROUP BY stadium;

-- 11. For every match involving 'POL', show the matchid, date and the number of goals scored.
SELECT matchid, mdate, COUNT(1) FROM game JOIN goal ON matchid = id 
 WHERE (team1 = 'POL' OR team2 = 'POL') GROUP BY matchid;

-- 12. For every match where 'GER' scored, show matchid, match date and the number of goals scored by 'GER'
SELECT matchid, mdate, COUNT(1) FROM game JOIN goal ON matchid = id JOIN eteam e ON teamid = e.id 
  WHERE (team1 = 'GER' OR team2 = 'GER') AND teamname = 'Germany' GROUP BY matchid; 

-- 13. Notice in the query given every goal is listed. If it was a team1 goal then a 1 appears in score1, 
